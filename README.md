...
Search and query [compact](https://doc.log10x.com/run/transform/#compact) Log10x events directly within Elasticsearch and OpenSearch with zero data loss. This open-source [Log10x](https://www.log10x.com/?utm_source=github&utm_medium=readme&utm_campaign=elasticsearch-plugin&utm_content=hero) plugin transparently expands compact events at query time, maintaining full search, Kibana, and alerting capabilities. On Elasticsearch 8.17.0 with the default LZ4 codec, a single shard, and the index force-merged, a typical structured-log workload with 200 to 500 byte bodies measured a 46 percent smaller on-disk index footprint from compact encode alone, and 65 percent smaller with engine-side envelope pruning enabled. See [Where the savings come from](#where-the-savings-come-from) for the measured figure for each body-size class, the encode-mode requirement, and the pruning recipe.
