...
Search and query [compact](https://doc.log10x.com/run/transform/#compact) Log10x events directly within OpenSearch with zero data loss. This open-source plugin transparently expands compact events at query time, maintaining full search and alerting capabilities. The on-disk reduction was measured on the Elasticsearch variant, on Elasticsearch 8.17.0 with the default LZ4 codec, a single shard, and the index force-merged: a typical structured-log workload with 200 to 500 byte bodies measured a 46 percent smaller on-disk index footprint from compact encode alone, and 65 percent smaller with engine-side envelope pruning enabled. OpenSearch 2.19.0 was not measured separately. See [Where the savings come from](../README.md#where-the-savings-come-from) in the main README for the measured figure for each body-size class, the encode-mode requirement, and the pruning recipe.
